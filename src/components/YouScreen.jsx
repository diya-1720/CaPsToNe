...
export const YouScreen = ({ 
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-3 pb-24 lg:pb-12 space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-wrap items-center justify-between glass-card p-5 sm:p-6 rounded-3xl border border-white/10 text-left gap-4">
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="space-y-0.5 min-w-0">
            ...
            <p className="text-xs sm:text-sm text-slate-300 font-light truncate">
              {currentUser?.email || 'guest@awen.app'}
            </p>
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-300 text-xs font-medium border border-emerald-500/20">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {currentUser ? (
            <button
              onClick={onLogout}
              className="p-2.5 sm:px-4 sm:py-2.5 rounded-xl glass-card hover:bg-rose-500/20 text-slate-300 hover:text-rose-300 border border-white/10 transition-colors flex items-center gap-2 text-xs font-semibold active:scale-95"
              title="Log Out"
            >
            ...
            <button
              onClick={onOpenAuth}
              className="px-4 py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-semibold shadow-md shadow-cyan-600/30"
            >
              Sign In / Up
            </button>
          )}
      ...
      <div className="glass-card p-6 sm:p-7 rounded-3xl border border-white/10 space-y-4 text-left shadow-xl">
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <span className="text-xs font-semibold uppercase tracking-wider text-cyan-300 font-mono flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="p-3.5 rounded-2xl bg-white/5 space-y-0.5">
            <span className="text-[10px] text-slate-400 font-mono block">Resting HR</span>
            <span className="text-lg font-heading font-bold text-white block">{restingHrVal} bpm</span>
            <span className="text-[9px] text-slate-400">Quiet Signature</span>
          </div>

          <div className="p-3.5 rounded-2xl bg-white/5 space-y-0.5">
            <span className="text-[10px] text-slate-400 font-mono block">Usual Variance</span>
            <span className="text-lg font-heading font-bold text-emerald-300 block">±{hrStdDevVal} bpm</span>
            <span className="text-[9px] text-slate-400">Expected Spread</span>
          </div>

          <div className="p-3.5 rounded-2xl bg-white/5 space-y-0.5">
            <span className="text-[10px] text-slate-400 font-mono block">Observation Days</span>
            <span className="text-lg font-heading font-bold text-cyan-300 block">{obsDay} / 7 Days</span>
            <span className="text-[9px] text-slate-400">Baseline History</span>
          </div>

          <div className="p-3.5 rounded-2xl bg-white/5 space-y-0.5">
            <span className="text-[10px] text-slate-400 font-mono block">Stair Filter</span>
            <span className="text-lg font-heading font-bold text-amber-300 block">+34.0 bpm</span>
            <span className="text-[9px] text-slate-400">Exertion Offset</span>
          </div>
        </div>
      </div>

      {/* 3. HOW AWEN IS LEARNING (Confidence Progress Card) */}
      <div className="glass-card p-6 sm:p-7 rounded-3xl border border-purple-500/30 space-y-4 text-left">
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          ...
          <span className={`text-[10px] sm:text-xs font-semibold px-2.5 py-0.5 rounded-full border ${isLearning ? 'bg-purple-500/20 text-purple-300 border-purple-500/30' : 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30'}`}>
            {confidenceState}
          </span>
        </div>
            ...
            {CONFIDENCE_TIERS.map((tier, idx) => {
              ...
              return (
                <div key={tier.name} className="space-y-1">
                  <div className={`h-2 rounded-full transition-all ${isCurrent ? 'bg-cyan-400 shadow-md shadow-cyan-400/50' : isPassed ? 'bg-emerald-400' : 'bg-slate-800'}`} />
                  <span className={`text-[10px] font-mono block truncate ${isCurrent ? 'text-cyan-300 font-bold' : isPassed ? 'text-slate-300' : 'text-slate-500'}`}>
                    {tier.name.split(' ')[0]}
                  </span>
                </div>
              );
      ...
      <div className="glass-card p-6 sm:p-7 rounded-3xl border border-white/10 space-y-3 text-left">
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          ...
          <span className={`text-[10px] sm:text-xs font-semibold px-2.5 py-0.5 rounded-full border ${telemetry?.isHardware ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-300' : 'bg-cyan-500/20 border-cyan-500/30 text-cyan-300'}`}>
        ...
        <button
          ...
          className="w-full py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs sm:text-sm font-medium text-slate-200 flex items-center justify-between transition-colors px-4"
        >
        ...
        <p className="text-xs sm:text-sm text-slate-300 leading-relaxed font-light italic bg-slate-900/40 p-4 rounded-2xl border border-white/5">
          ...
          <div className="space-y-3.5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] sm:text-xs text-slate-400">Baseline Progress</span>
            </div>
            ...
            <div className="space-y-3">
              {DISCOVERIES.map((disc, idx) => {
                ...
                return (
                  <div key={idx} className={`glass-card p-4 sm:p-5 rounded-2xl border transition-all ${isUnlocked ? 'border-white/10' : 'border-white/5 opacity-60'}`}>
                    <div className="flex items-center justify-between text-[10px] sm:text-xs">
                      <span className={`font-semibold uppercase tracking-wider ${isUnlocked ? 'text-cyan-400' : 'text-slate-500'}`}>{disc.day}</span>
                      <span className="text-slate-400">{disc.title}</span>
                    </div>
                    {isUnlocked ? (
                      <>
                        <p className="text-xs sm:text-sm italic text-slate-200 bg-slate-900/60 p-3 rounded-xl border border-white/5 font-light mt-1.5">
                          "{disc.quote}"
                        </p>
                        <p className="text-[11px] sm:text-xs text-slate-400 leading-normal pt-1 font-light">
                          {disc.detail}
                        </p>
                      </>
                    ) : (
                      <p className="text-xs text-slate-400 italic bg-slate-900/30 p-3 rounded-xl border border-white/5 font-light mt-1.5">
      ...
      {isIoTModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn">
          <div className="relative w-full max-w-md glass-card p-6 rounded-3xl border border-white/10 space-y-4 shadow-2xl text-left bg-[#0d1527] max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
            ...
            <div className="p-4 rounded-2xl bg-slate-900/60 border border-white/5 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-200">Physical Hardware Connection</span>
              ...
              {telemetryStream?.isHardwareConnected ? (
                <button
                  ...
                  className="w-full py-2 rounded-xl bg-rose-500/20 hover:bg-rose-500/30 text-rose-200 border border-rose-500/30 text-xs font-semibold transition-colors"
                >
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-semibold shadow-md shadow-cyan-600/30 transition-all flex items-center justify-center gap-2"
                >
            ...
            <div className="p-4 rounded-2xl bg-slate-900/60 border border-white/5 space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-200">Data Contract & Parser Test Suite</span>
                <button
                  onClick={handleRunSelfTest}
                  className="px-2.5 py-1 rounded-lg bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 border border-indigo-500/30 text-[10px] font-semibold transition-colors"
                >
              ...
              {testResults ? (
                <div className="space-y-1.5 pt-1">
                  {testResults.map((tr, idx) => (
                    <div key={idx} className="flex items-center justify-between text-[11px] p-2 rounded-lg bg-slate-950/40 border border-white/5">
                      <span className="text-slate-300 font-light">{tr.name}</span>
                      <span className={`font-mono text-[10px] font-semibold px-2 py-0.5 rounded ${tr.passed ? 'bg-emerald-500/20 text-emerald-300' : 'bg-rose-500/20 text-rose-300'}`}>
                        {tr.passed ? 'PASS' : 'FAIL'}
                      </span>
                    </div>
                  ))}
            ...
            <div className="space-y-2 text-xs pt-1">
              <label className="font-semibold text-slate-300">Simulate Physical Activity Scenario:</label>
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => { telemetryStream?.setScenario('normal'); setIsIoTModalOpen(false); }}
                  className="p-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-slate-200 border border-white/5 text-left"
                >
                  Resting (64 bpm)
                </button>
                <button
                  onClick={() => { telemetryStream?.setScenario('stairs'); setIsIoTModalOpen(false); }}
                  className="p-2.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/20 text-left"
                >
                  Stairs (+34 bpm)
                </button>
                <button
                  onClick={() => { telemetryStream?.setScenario('caffeine'); setIsIoTModalOpen(false); }}
                  className="p-2.5 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 border border-cyan-500/20 text-left"
                >
                  Caffeine Work
                </button>
                <button
                  onClick={() => { telemetryStream?.setScenario('exercise'); setIsIoTModalOpen(false); }}
                  className="p-2.5 rounded-xl bg-purple-500/10 hover:bg-purple-500/20 text-purple-300 border border-purple-500/20 text-left"
                >
                  Running
                </button>
              </div>
            ...
            <div className="space-y-2 text-xs pt-2 border-t border-white/10">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="p-2 rounded-xl bg-purple-500/10 hover:bg-purple-500/20 text-purple-200 border border-purple-500/20 text-left text-[11px]"
                >
                  Seed 58 bpm Profile
                </button>
                <button
                  ...
                  className="p-2 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-200 border border-cyan-500/20 text-left text-[11px]"
                >
                  Seed 76 bpm Profile
                </button>
              </div>
            ...
            <div className="pt-2 border-t border-white/10 flex justify-end">
              <button
                ...
                className="px-4 py-2 rounded-xl bg-slate-800 text-xs font-medium text-white hover:bg-slate-700 transition-colors"
              >
                Close
              </button>
            </div>
